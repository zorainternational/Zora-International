...
/******************************************************************************************************************************
*    
*     Plugin Name: WooCommerce SADAD- PayTabs
...
For Live Merchants: You can use the website that you have submitted in the Go-Live application. If you need to edit/change the site URL, you can send a request to customercare@paytabs.com.','woocommerce' ),
  ...
  function get_paytabs_args( $order ) {
        ...
        $paytabs_args = array(
          ...
          'ip_merchant'     =>  $_SERVER['REMOTE_ADDR'],
          "return_url"      =>  $redirect,
          'cms_with_version' =>'Sadad WooCommerce:'.WOOCOMMERCE_VERSION,
          'reference_no'     => $txnid,
        ...
        if ( 'yes' == $this->send_shipping ) {
              $paytabs_args['address_shipping']    = $order->get_shipping_address_1().' '.$order->get_shipping_address_2();          
              $paytabs_args['city_shipping']       = $order->sget_hipping_city();
              $paytabs_args['state_shipping']      = $order->get_shipping_state() ;
              $paytabs_args['country_shipping']    = $this->getCountryIsoCode($order->get_shipping_country());
              $paytabs_args['postal_code_shipping']= $order->get_shipping_postcode();
        } else {
              $paytabs_args['address_shipping']     = $order->get_shipping_address_1().' '.$order->get_shipping_address_2();          
              $paytabs_args['city_shipping']       = $order->get_shipping_city();
              $paytabs_args['state_shipping']      = $order->get_shipping_state();
              $paytabs_args['country_shipping']    = $this->getCountryIsoCode($order->get_shipping_country());
              $paytabs_args['postal_code_shipping']= $order->get_shipping_postcode();
